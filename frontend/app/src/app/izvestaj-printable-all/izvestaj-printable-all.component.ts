...
@Component({
  selector: 'app-izvestaj-printable-all',
  templateUrl: './izvestaj-printable-all.component.html',
  styleUrls: ['./izvestaj-printable-all.component.css']
})
export class IzvestajPrintableAllComponent {
  ...
  ngOnInit(): void {
    ...
    this.korisnikService.login(this.korisnik.kor_ime, this.korisnik.lozinka).subscribe((blueprint: Korisnik)=>{
      ...
      this.izvestaji = JSON.parse(sessionStorage.getItem('izvestaji'))
    })
  ...
  public downloadAsPDF() {
    ...
    pdfDocGenerator.getBlob((blob) => {
      ...
      for(let k = 0; k < this.izvestaji.length ; ++k){
        naziv += this.izvestaji.at(k).datum
        naziv += this.izvestaji.at(k).vreme
        naziv += this.izvestaji.at(k).lekar
      }
      naziv += JSON.parse(sessionStorage.getItem('korisnik')).kor_ime
      naziv = naziv + ".pdf"
      const emailData = {
  ...
  nazad(){
    sessionStorage.removeItem('izvestaji')
